...
export async function helpChat(conversationHistory, challenge, department = null) {
  ...
  // Get department-specific active workflows
  const departmentActiveWorkflows = department
    ? companyContext.activeWorkflows
        .filter(w => w.team === department)
        .map(w => `• ${w.workflow} using ${w.tool}: ${w.description}`)
        .join('\n')
    : null;
...
Be conversational and concise (this is Slack). Use Slack mrkdwn formatting. Include clickable links using Slack format: <URL|Link Text>`;
