...
let workflowsCacheTimestamp = 0;
const WORKFLOWS_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

// Get workflows from the "Workflows" sheet tab
// Columns: A=Team, B=Workflow, C=Tool, D=Description, E=Grade (A/B/C/D)
export async function getWorkflows() {
  if (workflowsCache && (Date.now() - workflowsCacheTimestamp) < WORKFLOWS_CACHE_TTL) {
  ...
  try {
    ...
    const workflows = rows.slice(1).map(row => ({
      team: row[0] || '',
      workflow: row[1] || '',
      tool: row[2] || '',
      description: row[3] || '',
      grade: (row[4] || 'C').toUpperCase().charAt(0) // Default to C if no grade
    })).filter(w => w.team && w.workflow) // Skip empty rows
      .sort((a, b) => (gradeOrder[a.grade] ?? 3) - (gradeOrder[b.grade] ?? 3)); // Sort A first

    workflowsCache = workflows;
    workflowsCacheTimestamp = Date.now();

    console.log(`Loaded ${workflows.length} workflows from sheet`);
    return workflows;
  } catch (error) {
    console.error('Failed to read workflows:', error.message);
    return workflowsCache || [];
  }
}

// Seed the Workflows tab with initial data from config (run once)
export async function seedWorkflows(activeWorkflows) {
  const client = await getSheetsClient();
  ...
  try {
    ...
    const header = ['Team', 'Workflow', 'Tool', 'Description', 'Grade'];
    const dataRows = activeWorkflows.map(w => [w.team, w.workflow, w.tool, w.description, w.grade || 'B']);
    ...
    console.log(`Seeded ${dataRows.length} workflows to sheet`);
    return true;
  } catch (error) {
    console.error('Failed to seed workflows:', error.message);
    return false;
  }
}
