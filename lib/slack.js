...
export async function postToChannel(text, channelId = null) {
  const slack = getClient();
  const channel = channelId || process.env.SLACK_ANNOUNCEMENTS_CHANNEL_ID || process.env.SLACK_CHANNEL_ID;
  if (!channel) {
